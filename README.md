...
The redesigned **wall-first** console now serves at **`/`** (with **`/console`** kept as an alias); the old flat page has been retired. Each Surface gets a 2D **wall plan** that draws its Frames as rectangles from their millimetre geometry (`x_mm/y_mm/width_mm/height_mm`), so you read the physical layout instead of a table; legacy Frames created at the origin with no distinct geometry collect in an **Unplaced tray** beside the plan. Selecting a Frame opens a read-only **Frame Inspector** with three facets — **Commissioning** (the display behind the Frame), **Binding** (the Player/Output serving it), and **Now-showing**. The now-showing tile is deliberately honest: it reads **_Scheduled: `<scene_id>`_** with the Run phase and a real connectivity dot, because it reports what central *intends* to show plus whether the Player is connected — it never claims confirmed playback and never labels a Frame "LIVE." The **Commissioning** facet can now **calibrate the display**: drag the corner and crop handles (a folded quad or empty crop snaps back), preview under a 30-second server lease with a visible countdown, then commit or revert — one shared preview slot, last-writer-wins, with explicit conflict states (see the [runbook](docs/runbook.md#operator-console-commissioning-calibration-and-conflict-states)). New or replacement Players enroll unbound into a **Pending rail**; you bind one from the **Binding** facet (a bind forces a "Review required" re-commission but never discards committed calibration), and a returning known Pi reappears already bound with a plain "recovered by serial match, not identity" banner (see [onboarding, binding, and auto-recovery](docs/runbook.md#operator-console-onboarding-binding-and-auto-recovery)). You now **edit the wall plan by direct manipulation**: drag a rectangle on the empty plan to **place** a new Frame (then enter its display profile), drag an existing rectangle to **move** one (last-write-wins, never a resize), drag a Frame out of the **Unplaced tray** to give it a position, or **delete** a Frame that is neither bound nor targeted by a live Run (see [placing, moving, and deleting Frames](docs/runbook.md#operator-console-placing-moving-and-deleting-frames-and-the-unplaced-tray)). A **Wall / Showrunner** toggle (organization, not permission — the same single admin token) switches to **Showrunner mode**, where you manage **Sources** (saved live `name:rev` queries — create, list, and Refresh; no album/Immich language), author **Scenes** (live-source or per-Frame authored, with profile-filtered choosers), schedule **Programs** (single windows plus priority), and drive **Runs** (activate now and read the synchronous `{status, reason}`; finish or cancel a live Run; a "why" panel ranks a Frame's contributions by precedence). Showrunner deliberately cannot reach Commissioning — at show time the only hardware fact it surfaces is each Frame's `calibration_valid` health badge (see [running the show](docs/runbook.md#operator-console-showrunner-running-the-show)).
