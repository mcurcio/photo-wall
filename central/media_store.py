...
class ReadLease:
    ...
    def __init__(self, store, fd: int, variant: Variant, owner: str, start: tuple[float, float]):
        ...
        self._timer = threading.Timer(max(.001, _READ_SECONDS - (time.monotonic() - start[0])), self.close)
        ...
        with self._lock:
            ...
            try:
                ...
                if (current.st_size, current.st_mtime_ns) != (
                    self._initial_stat.st_size, self._initial_stat.st_mtime_ns
                ):
...
class MediaStore:
    ...
    def _open(self, path: Path) -> int:
        ...
        try:
            for name in relative.parts[:-1]:
                ...
                os.close(parent)
                parent = child
            return os.open(relative.name, os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK, dir_fd=parent)
        finally:
            os.close(parent)
    ...
    def _hash(self, fd: int, maximum: int, start=None) -> tuple[int, str, str]:
        ...
        while chunk := os.read(fd, 65536):
            ...
            sha256.update(chunk)
        after = os.fstat(fd)
        if (count, before.st_size, before.st_mtime_ns) != (after.st_size, after.st_size, after.st_mtime_ns):
            raise MediaStoreError("media_corrupt", 503)
        os.lseek(fd, 0, os.SEEK_SET)
    ...
    def _refresh(self, conn, digest: str):
        sources = conn.execute("SELECT DISTINCT s.source_ref,s.status FROM media_sources s "
                               "JOIN source_members m ON m.source_ref=s.source_ref JOIN media_jobs j "
                               "ON j.asset_id=m.asset_id WHERE j.variant_sha=%s", (digest,)).fetchall()
        for source in sources:
            self.repository._refresh_catalog(conn, source["source_ref"], self.clock.utc(), source["status"])
    ...
    def _mark_corrupt(self, digest: str):
        ...
        with self.repository.transaction() as conn:
            conn.execute("UPDATE media_blobs SET state='corrupt',size=GREATEST(size,%s) "
                         "WHERE digest=%s AND state<>'deleting'", (size, digest))
            self._refresh(conn, digest)

    def _checked(self, conn, lease):
        row = conn.execute("SELECT * FROM media_jobs WHERE id=%s AND attempt_token=%s AND lease_until>%s",
                           (lease.job_id, lease.attempt_token, self.clock.utc())).fetchone()
        if not row or row["asset_id"] != lease.asset.asset_id or row["recipe_id"] != lease.recipe_id:
            raise MediaStoreError("stale_job")
        return row
    ...
    def publish(self, lease: JobLease, prepared: PreparedMedia) -> Variant:
        with self._writer():
            ...
            cap = (self.repository.limits.max_image_bytes if lease.asset.kind == "image"
                   else self.repository.limits.max_video_bytes)
            if variant.size > cap or variant.media_type != ("image/jpeg" if lease.asset.kind == "image" else "video/mp4"):
                raise MediaStoreError("media_result_mismatch")
            try:
                fd = self._open(paths.original)
                try:
                    original_size, sha1, sha256 = self._hash(fd, self.repository.limits.max_original_bytes)
                finally:
                    os.close(fd)
                if (sha1 != lease.asset.original_sha1 or sha256 != prepared.original_sha256
                        or (lease.asset.file_size is not None and original_size != lease.asset.file_size)):
                    raise MediaStoreError("media_original_mismatch")
                self._verify(paths.variant, variant)
                with self.repository.transaction() as conn:
                    row = self.repository.checked_job(conn, lease)
                    if row["state"] != "running" or row["reserved_bytes"] < original_size + variant.size:
                        raise MediaStoreError("stale_job")
                    existing = conn.execute("SELECT * FROM media_blobs WHERE digest=%s", (variant.sha256,)).fetchone()
                    if existing:
                        if existing["state"] != "ready" or existing["variant"] != variant.model_dump(mode="json"):
                            raise MediaStoreError("media_blob_unavailable", 503)
                        self._verify(self._blob(variant.sha256), variant)
                    else:
                        conn.execute("INSERT INTO media_blobs VALUES(%s,%s,%s,'publishing',%s)",
                                     (variant.sha256, Jsonb(variant.model_dump(mode="json")), variant.size, self.clock.utc()))
                    result = {**prepared.model_dump(mode="json", exclude={"path"}),
                              "asset_id": lease.asset.asset_id, "original_sha1": lease.asset.original_sha1}
                    conn.execute("UPDATE media_jobs SET state='publishing',variant_sha=%s,result=%s,"
                                 "reserved_bytes=reserved_bytes-%s,cleanup_state='ready',updated_at=%s WHERE id=%s",
                                 (variant.sha256, Jsonb(result), 0 if existing else variant.size, self.clock.utc(), lease.job_id))
                if not existing:
                ...
                with self.repository.transaction() as conn:
                    ...
                    conn.execute("UPDATE media_jobs SET state='cleanup' WHERE id=%s", (lease.job_id,))
    ...
    def _lease_for(self, conn, row) -> JobLease:
        ...
        asset = conn.execute("SELECT metadata FROM asset_revisions WHERE asset_id=%s", (row["asset_id"],)).fetchone()
        return JobLease(job_id=row["id"], attempt_token=row["attempt_token"],
                        asset=OriginalAsset.model_validate(asset["metadata"]), recipe_id=row["recipe_id"],
                        lease_until=row["lease_until"], reserved_bytes=max(1, row["reserved_bytes"]))
    ...
    def _cleanup(self, row, *, lease=None):
        ...
        if row["cleanup_state"] == "ready":
            result = row["result"]
            if (not result or result["recipe_id"] != row["recipe_id"]
        ...
        self._fsync_directory(paths.directory.parent if paths.directory.parent.exists() else self.root / "staging")
        with self.repository.transaction() as conn:
            current = self._checked(conn, lease) if lease else conn.execute(
                "SELECT * FROM media_jobs WHERE id=%s AND attempt_token=%s", (row["id"], row["attempt_token"])).fetchone()
            if not current:
                raise MediaStoreError("stale_job")
            ...
            if target not in ("ready", "retry", "failed"):
                raise MediaStoreError("media_journal_invalid", 503)
            if target == "ready":
                blob = conn.execute("SELECT state,variant,size FROM media_blobs WHERE digest=%s",
                                    (current["variant_sha"],)).fetchone()
                if (not blob or blob["state"] not in ("ready", "publishing")
                        or blob["variant"] != current["result"]["variant"]
                        or blob["size"] != current["result"]["variant"]["size"]):
                    raise MediaStoreError("media_journal_invalid", 503)
                conn.execute("UPDATE media_blobs SET state='ready' WHERE digest=%s AND state='publishing'",
                             (current["variant_sha"],))
            conn.execute("UPDATE media_jobs SET state=%s,reserved_bytes=0,cleanup_state=NULL,lease_until=NULL,"
                         "updated_at=%s WHERE id=%s", (target, self.clock.utc(), row["id"]))
            if current["variant_sha"]:
                self._refresh(conn, current["variant_sha"])

    def fail(self, lease: JobLease, code: str, *, retry_at: float | None = None):
        if not isinstance(code, str) or not _CODE.fullmatch(code):
            raise MediaStoreError("media_failure_code", 400)
        if retry_at is not None and (not math.isfinite(retry_at) or retry_at <= self.clock.utc()):
            raise MediaStoreError("media_retry_time", 400)
        with self._writer():
            with self.repository.transaction() as conn:
                row = conn.execute("SELECT * FROM media_jobs WHERE id=%s AND attempt_token=%s "
                                   "AND state IN ('running','publishing','cleanup')",
                                   (lease.job_id, lease.attempt_token)).fetchone()
                if not row or row["asset_id"] != lease.asset.asset_id or row["recipe_id"] != lease.recipe_id:
                    raise MediaStoreError("stale_job")
                if row["variant_sha"]:
                    blob = conn.execute("SELECT state FROM media_blobs WHERE digest=%s", (row["variant_sha"],)).fetchone()
                    if blob and blob["state"] == "publishing":
                        conn.execute("UPDATE media_blobs SET state='corrupt' WHERE digest=%s", (row["variant_sha"],))
                conn.execute("UPDATE media_jobs SET state='cleanup',cleanup_state=%s,retry_at=%s,"
                             "failure_code=%s,updated_at=%s WHERE id=%s RETURNING *",
                             ("failed" if retry_at is None else "retry", retry_at or 0, code,
                              self.clock.utc(), row["id"]))
    ...
    def _tree_size(self, path: Path, *, depth=0, budget=None) -> int:
        ...
        if stat.S_ISDIR(info.st_mode):
            return sum(self._tree_size(child, depth=depth + 1, budget=budget) for child in path.iterdir())
        return info.st_size
    ...
    def _orphan(self, path: Path) -> bool:
        ...
        with self.repository.transaction() as conn:
            conn.execute("INSERT INTO media_orphans(path,size) VALUES(%s,%s) "
    ...
    def recover(self) -> RecoveryReport:
        with self._writer():
            ...
            with self.repository.transaction() as conn:
                rows = conn.execute("SELECT * FROM media_jobs WHERE reserved_bytes>0 OR state IN "
                                    "('running','publishing','cleanup') ORDER BY id LIMIT %s", (_SCAN_LIMIT + 1,)).fetchall()
            ...
            for row in rows:
                ...
                try:
                    ...
                    if row["cleanup_state"] == "ready" and row["result"]:
                        ...
                        with self.repository.transaction() as conn:
                            row = conn.execute("UPDATE media_jobs SET state='cleanup',cleanup_state='retry',"
                                               "retry_at=%s,failure_code='worker_interrupted' WHERE id=%s RETURNING *",
                                               (self.clock.utc() + 5, row["id"])).fetchone()
                    self._cleanup(row)
                    jobs += 1
                    ...
                    try:
                        self.fail(lease, "publication_incomplete", retry_at=self.clock.utc() + 5)
                    except (OSError, MediaStoreError):
                        pending += 1
            with self.repository.transaction() as conn:
                blobs = conn.execute("SELECT * FROM media_blobs WHERE state='ready'").fetchall()
                abandoned = conn.execute("SELECT digest FROM media_blobs b WHERE state='publishing' "
                                         "AND NOT EXISTS(SELECT 1 FROM media_jobs j WHERE j.variant_sha=b.digest "
                                         "AND j.state IN ('publishing','cleanup'))").fetchall()
            for blob in abandoned:
                self._mark_corrupt(blob["digest"])
            for blob in blobs:
                try:
                    self._verify(self._blob(blob["digest"]), Variant.model_validate(blob["variant"]))
                except (OSError, MediaStoreError, ValueError):
                    self._mark_corrupt(blob["digest"])
                    corrupt += 1
            self.collect()
            with self.repository.transaction() as conn:
                known_blobs = {row["digest"] for row in conn.execute("SELECT digest FROM media_blobs").fetchall()}
                active = {(row["id"], row["attempt_token"]) for row in conn.execute(
                    "SELECT id,attempt_token FROM media_jobs WHERE reserved_bytes>0").fetchall()}
                tracked = [self.root / row["path"] for row in conn.execute("SELECT path FROM media_orphans").fetchall()]
            ...
            unknown += [p for p in children(self.root) if p.name not in (".worker.lock", "blobs", "staging")]
    ...
    def collect(self, *, target_bytes: int | None = None) -> GCReport:
        with self._writer():
            ...
            with self.repository.transaction() as conn:
                ...
                rows = conn.execute("SELECT * FROM media_blobs WHERE state IN ('ready','corrupt','deleting') "
                                    "ORDER BY CASE WHEN state='ready' THEN 1 ELSE 0 END,created_at,digest").fetchall()
            for row in rows:
                with self.repository.transaction() as conn:
                    if row["state"] == "ready" and self.repository.accounted_bytes(conn) <= target:
                        continue
                    if conn.execute("SELECT 1 FROM media_references WHERE digest=%s AND expires_at>%s",
                                    (row["digest"], self.clock.utc())).fetchone():
                        continue
                    if conn.execute("SELECT 1 FROM media_jobs WHERE variant_sha=%s AND cleanup_state='ready'",
                                    (row["digest"],)).fetchone():
                        continue
                    changed = conn.execute("UPDATE media_blobs SET state='deleting' WHERE digest=%s "
                                           "AND state IN ('ready','corrupt','deleting') RETURNING digest", (row["digest"],)).fetchone()
                    if not changed:
                        continue
                ...
                with self.repository.transaction() as conn:
                    ...
                    conn.execute("UPDATE media_jobs SET variant_sha=NULL,state=CASE WHEN state='ready' "
                                 "THEN 'evicted' ELSE state END,retry_at=%s,updated_at=%s WHERE variant_sha=%s",
                                 (self.clock.utc(), self.clock.utc(), row["digest"]))
                    conn.execute("DELETE FROM media_blobs WHERE digest=%s AND state='deleting'", (row["digest"],))
                removed += 1
            with self.repository.transaction() as conn:
    ...
    def open_read(self, token: str, digest: str) -> ReadLease:
        ...
        try:
            with self.db.transaction() as conn:
                ...
                observed = self.registry.configuration_in(conn, identity["id"], identity["authority_epoch"])
                configuration = PlayerConfiguration(
                    player_id=identity["id"], authority_epoch=identity["authority_epoch"], configuration_revision=1,
                ...
                blob = conn.execute("SELECT * FROM media_blobs WHERE digest=%s AND state='ready'", (digest,)).fetchone()
                if not blob:
                    raise MediaStoreError("media_unavailable", 404)
                variant, now = Variant.model_validate(blob["variant"]), self.clock.utc()
                refs = {row["owner"] for row in conn.execute(
                    "SELECT owner FROM media_references WHERE digest=%s AND expires_at>%s", (digest, now)).fetchall()}
                authorized = False
                offers = conn.execute("SELECT manifest FROM plan_offers WHERE player_id=%s AND authority_epoch=%s "
                                      "AND valid_until>%s", (identity["id"], identity["authority_epoch"], now)).fetchall()
                for row in offers:
                    plan = Plan.model_validate(row["manifest"])
                    reference = f"offer:{plan.player_id}:{plan.authority_epoch}:{plan.revision}"
                    if reference in refs and any(layer.variant == variant and layer.end > now
                                               and configuration.authorizes(layer) for layer in plan.layers):
                        authorized = True
                locks = conn.execute("SELECT assignment_id,layer FROM assignment_locks WHERE player_id=%s "
                                     "AND authority_epoch=%s AND valid_until>%s",
                                     (identity["id"], identity["authority_epoch"], now)).fetchall()
                for row in locks:
                    reference = f"secured:{identity['id']}:{identity['authority_epoch']}:{row['assignment_id']}"
                    layer = Layer.model_validate(row["layer"])
                    if (reference in refs and layer.variant == variant and layer.end > now
