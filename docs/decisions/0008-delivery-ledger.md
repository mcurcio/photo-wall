...
| p4-boot-chain **s3** (process-level e2e tracer) | **owner-chosen shape (2026-09-12): process-level, NOT QEMU kernel-boot.** Part A: real central+Postgres serves the REAL player .deb; the REAL Bootstrapper(provision.py) fetches + sha256-verifies (+ flip-a-byte refusal) + installs it in a container + writes origin handoff + starts the unit (asserts install landed). Part B: headless player enrolls TICKETLESS (re-key demo_wall off /v1/bootstrap/boot) -> pending -> operator bind -> render-path smoke (RecordingRenderer). Discovery injected via explicit central_origin (no multicast). GATES all deletion. Leaves real GTK/HDMI pixels + real kernel/initrd boot to owner Pi. | CI (arm64, no VM) | open (next) |
...
| p4-deb-full-depends | .deb declares full app Depends; bootstrapper apt-installs it | yes | open — **SCOPED (2026-09-12): frame-level.** Option (b): DROP the venv+wheelhouse+chroot; .deb Depends = native (GTK/GStreamer/weston/Mesa) + python3-* (pydantic/httpx/websockets/cryptography/zeroconf/gi/gst/opengl) resolved from trixie at boot; provision.py dpkg-i -> `apt-get install -y ./player.deb`. Forces Python **3.12 -> 3.13** (trixie has no python3.12; wheelhouse is cp312-pinned) and trades hash-pinned deps for distro versions (OK per no-threat-model). RISK: app-compat on 3.13 + unpinned versions — runtime/hardware validates. CI proof: build .deb on plain trixie mmdebstrap root + `apt-get install --simulate ./player.deb` resolves; s3 Part A installs for real. |
