...
Owner chose FULL (2026-09-12): migrate the media-OS test base off the old pipeline FIRST, then delete the whole pipeline. Order: **p4-media-os-migrate -> s5(full) -> s4 -> s6. ALL LANDED + CI-GREEN.**
| Slice | Delivers | CI? | Status |
|---|---|---|---|
| p4-media-os-migrate | made scripts/service_base.py self-contained (inlined the OCI-registry helpers it borrowed from ci_images), severing the media-OS test base from appliance.build/os_base | software-e2e + checks green | **closed `5e65026`** (+ build_player 3.13 fix `3fa87e2`) |
| p4-retire **s5** (build pipeline, FULL) | deleted appliance/build.py + os_packages + os_definition.json + os_base + fetch_ubuntu + ci_base_cache + ci_apt_cache + ci_images + build_ci_*/vm_*/boot_gateway/boot_fixture/rollback + appliance/initramfs/ + appliance.yml + 26 tests; rewired release.yml/base-image.yml/package_release_artifacts to publish {base bundle, player .deb, bootstrapper .deb} | checks+base-image+software-e2e green | **closed `1b99053`** (-16,599 lines) |
| p4-retire **s4** (signed client+central) | deleted central/releases.py + release_models.py + appliance/updates.py + trial systemd units + demo_registry + their tests; reduced appliance/bootstrap.py to netboot essentials + contracts/release.py to MAX_ROOTFS_BYTES; removed signed routes/release-authority from central/app.py + registry ticket branch + player boot-health/release_accepted; demo_wall off signed staging | checks+software-e2e green | **closed `e0f4f77`** (-2,536 lines) |
| p4-retire **s6** (drop tables, FINAL) | migration 015 DROP the five appliance_* tables; removed the obsolete appliance_devices test | migration applied vs real Postgres (checks) + software-e2e green | **closed `7c6f47a`** (irreversible; owner-authorized) |

### Residuals (follow-up beads — not blocking; the new path is fully green)
- **Docs rewrite bead:** the retired-pipeline module docs (module-appliance-builder/ci/e2e/release, module-boot-fixture, module-pxe-service prose, runbook.md, and README's flash-image + signed-path sections) still DESCRIBE the deleted pipeline. s5/s4 only de-linked dead file references so check_docs passes; the prose is stale and needs a rewrite to the 0009 base-bundle + .deb-from-central flow. README was deliberately untouched (parallel readme-usage-instructions work owns it).
- **Owner hardware validation (the real gate for the risks CI can't cover):** netboot the published base bundle on a real Pi 5; confirm the player .deb apt-installs its deps + renders on Python 3.13 + trixie distro versions on real HDMI. CI proved deps RESOLVE + imports load on 3.13, not real pixels/boot.
- Minor: scripts/verify_netboot_initrd.py forbidden-list still names appliance/updates.py (now a deleted file — harmless string assertion); a few stale prose comments reference _report_boot_health/boot()/release_accepted; appliance/bootstrap.py keeps an unused self._watchdog_fd.
...
- **Transport split (decided):** kernel + initrd over TFTP; large squashfs over HTTP. **Superseded by central-discovery (`feat/netboot-central-discovery`):** cmdline.txt now carries ONLY `photowall.central=` (Central's ROOT URL, set once per site, never baked) — the base image is fleet-wide immortal as the squashfs is revised. The netboot request path is the code constant `/v1/netboot/base` appended by the initrd; the Pi self-identifies with its hardware serial in the `X-PhotoWall-Serial` request header; and the corruption digest arrives in the HTTP `Digest: sha-256=<base64>` response header (a missing/mismatched digest fails closed). The old `photowall.base_url`/`photowall.base_sha256` cmdline params are removed.
