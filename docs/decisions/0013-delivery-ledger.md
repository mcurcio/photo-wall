...
| B5 | Retire hand-staging: `register_app`/`promote_app` → **410 Gone** tombstones; drop unused models; KEEP `AppPackages.register/.promote` (mirror+reconcile untouched); hermetic 410 test (probe 7); docs + release.yml cleanup | 7 | **code-ready on `wip/`, merge-BLOCKED (owner)** | wip/0013-b5-hand-staging-removal @ 16a9054 | Implemented + locally verified (1017/331, 410 test green) but deliberately on a wip branch, NOT in the deployable PR. **Merge gated on the decision-5 precondition**: the SQL below must return 0 un-refetchable served `.deb`s (live-DB, owner-run). 0 rows → merge wip into the PR. Non-empty → drain first. Deleting hand-staging removes the re-stage recovery path, so it must not deploy until proven |
...
**Remaining (owner-gated — all code is written; these are pure owner actions):**
- **B5 (probe 7)** — hand-staging retirement is **implemented + verified on `wip/0013-b5-hand-staging-removal` @ 16a9054**, deliberately NOT in the deployable PR. Merge it into PR #19 only after the precondition SQL (below) returns 0 rows (live-DB, owner-run). Verified the bypass is real & live: `register_app` (app.py:709) stages `.deb` bytes out-of-band + records a pointer with no `app_releases` link; `promote` will promote such a sha. So a served sha CAN be un-refetchable — confirm none is before the routes retire and eviction is trusted.
- **iac** (separate repo) — one `cache` PVC; needs Slice 1 merged + the published image tag to pin. Brief fully grounded (scratchpad `iac-brief.md`); the iac worktree is currently dirty on unrelated work, so hold until the owner is ready. Pure structural change (PVC collapse + test rewrites) is tag-independent; only the tag bump waits on merge.
