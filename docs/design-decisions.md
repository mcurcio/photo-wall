...
| D10 | Fleet provisioning and trust | **Resolved by [decision 0008](decisions/0008-generic-image-and-serial-identity.md) (2026-09-10):** two-axis progressive provisioning — a serial-identity, mDNS-discovered, flashed-generic-image baseline (assumes only a LAN), with netboot and an opt-in certificate identity tier as independent enhancements. The image is fully generic and GitHub-Releases-hosted; all customization lives outside the image build. Remaining: qualification, and the per-tier implementation itself. | S3 automatic appearance/provisioning/replacement; S6 rollout. |
