...
Use synchronized system clocks and a defined mapping into local monotonic/media time. Specify clock-step, uncertainty and late-command behavior. Measure visible start skew and drift across actual panels; clock agreement or a successful synchronized start alone proves neither. Tight synchronization remains a qualification decision; do not promise frame accuracy from software timestamps.
...
- Support discovery, registration, identity establishment and initial equipment reporting according to [automatic Player provisioning](requirements.md#player-provisioning); represent an unbound Player in the Control Plane before issuing Frame execution authority.
...
|---|---|---|
| Registry | Registration/enrollment state, credentials, equipment observations, desired configuration, current/retired bindings and generations. | Automatic registration preserves unbound visibility; replacement preserves Frames and the old Player cannot recover stale authority. |
...
| Player | Accepted local configuration/plan identity, cache integrity/index and state needed by local continuation policy. | Restart recovers current authorized output or fallback rather than replaying expired instructions. |
...
On reconnect, exchange accepted epochs/revisions, current bindings, secured assets and observed output. The active central authority determines which present/future work remains valid and sends the state needed to converge. A log of missed commands is insufficient: the Player may have been replaced, an overlay may have completed, and underlying media may have advanced.

## Failure behavior

Preserve last-known-good configuration and use the configured local continuation/fallback policy during server, source or network failure. Losing the server alone must not imply blanket darkness. Define behavior beyond lookahead, offline schedule/overlay expiry, cache pressure and return to central authority; retained content must still satisfy compatibility.

Keep an empty eligible pool distinct from permission, API, transport, conversion and playback failures. Use the last valid still for an empty eligible pool by default, or configured fallback when none exists. Protect cached bytes for current playback and every scheduled-and-secured locked assignment, including any interval before formal commitment. Release those pins only when the execution no longer requires them; report inability to secure additional work instead of evicting required bytes.

Failures affecting completion return to Runtime. Natural completion waits for own work and children to finish or be cancelled; downward cancellation remains distinct. Program expiry stops new cycles and requests completion of current activity without draining prefetch. Actuator adapters apply only currently authorized contributions and report outcomes through the same ownership model.
