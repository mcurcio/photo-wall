...
The [native-media helper](../scripts/vm_media_probe.py) owns `VM_PHOTO_FRAME`, an explicit typed Frame definition: 1920×1080 pixels, 24-inch diagonal, video disabled, and a 500×281.25 mm aperture (500 mm long edge, 16:9). It binds that authored Frame to the currently connected Output identity. Its strict `MediaConfigurationReceipt` retains `authored_frame` and `observed_output` separately and checks the current Player and authority epoch. A discovered 0×0 mode remains unknown in the Output observation; it does not supply or replace authored Frame dimensions. GTK surface allocation and native committed-photo presentation require their own evidence after configuration.
