...
[Decision 0012](decisions/0012-netboot-base-auto-mirror.md) adds automatic base-image mirroring and a **per-device** whole-release path on top of the 0009/0010 contract above (which is left untouched — `GET /v1/app/manifest` and its `promoted_tag`/`current_sha256` pointer are neither read nor repurposed). Central discovers every release, caches each needed base squashfs on a dedicated `PHOTO_WALL_BASE_ROOT` volume, and serves each device the base of the version it resolves to. Two operational notes for this module:
