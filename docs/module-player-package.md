...
The central side records a promoted `{version, sha256, size}` pointer and serves those bytes by reference. Hand-staging is **retired** ([decision 0013](decisions/0013-unified-cache-root.md), decision 5): the `.deb` is sourced solely from GitHub releases, so the former operator routes `POST /v1/operator/app` and `PUT /v1/operator/app/current` now return **410 Gone** (there is no `PHOTO_WALL_APP_ROOT`). The recording/promotion happens automatically as central mirrors a promoted GitHub release — see [operator release sourcing](#operator-release-sourcing-0010) below, [the app-package contract](module-appliance-release.md#the-app-package-contract-0009-unsigned-in-parallel), and the [runbook](runbook.md#player-provisioning-netboot-and-promote-the-app-0009-in-progress). The sha256 involved is a corruption check only, per the owner's home-LAN ruling — there is no signing anywhere in this path.
...
The operator flow is three admin routes on central — `GET /v1/operator/app/releases` (list tracked releases), `POST /v1/operator/app/releases/{tag}/promote` (200 already-mirrored, 202 pending mirror, 404 unknown tag, 409 undeployable), and `POST /v1/operator/app/releases/refresh` (poll now). Release sourcing is **always-on** as of [decision 0013](decisions/0013-unified-cache-root.md) — the old opt-in gate (and its `503 release_sourcing_unconfigured` branch) is removed; the worker and central share the one cache root and the worker mirrors into its `apps/` subdir unconditionally. The [runbook](runbook.md#player-provisioning-promote-a-release-from-github-0010) has the copy-pasteable env config and `curl` commands. The former manual `POST /v1/operator/app` + `PUT /v1/operator/app/current` hand-staging path is **retired** (410 Gone) as of [decision 0013](decisions/0013-unified-cache-root.md) — GitHub is the sole `.deb` source, so every served `.deb` is re-fetchable and the `apps/` store is a true cache.
