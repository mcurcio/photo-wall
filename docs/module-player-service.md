...
The boot context binds `ticket_id`, centrally recognized `device_id`, Linux `boot_id`, selected `release_id`, and trial status. On netboot (D1), the common bootstrap produces it at `/run/photo-wall/boot.json` with `persistence="volatile"`, describing the intentional stateless model rather than a storage fault. On a flashed (D0) Player, that file does not exist — there is no boot server to write it — so the service synthesizes an equivalent boot context itself from the Pi's hardware serial (0008 baseline), with `persistence="persistent"`; `device_id` is derived from the same serial-hashing scheme either way, so one Pi keeps one `device_id` across delivery tiers. A D0 context carries no real netboot ticket, so enrollment sends `ticket_id=None` for it rather than the synthesized placeholder, which central's registry reads as the explicit D0 signal to enroll the Player unbound by serial with no prior boot ticket required.
