...
**The base squashfs is HTTP-served and auto-mirrored (0012).** Under [decision 0012](decisions/0012-netboot-base-auto-mirror.md) the operator no longer hand-stages the base squashfs: only the kernel/initrd/DTB (`config.txt`, `cmdline.txt`) are staged in the TFTP tree above, while the initramfs fetches the **base squashfs over HTTP** from central at `GET /v1/netboot/base` (bytes + a corruption `Digest`, self-identified by hardware serial). Central's worker discovers every release, downloads and verifies each needed release's squashfs onto the **single cache root's `os-images/` subdir (`PHOTO_WALL_CACHE_ROOT`, worker-RW/central-RO — [decision 0013](decisions/0013-unified-cache-root.md), which retired the per-domain `PHOTO_WALL_BASE_ROOT`)**, and serves the version each device resolves to. Base serving is always-on. A base miss is a **transient `503` that self-heals** (background fetch + reboot-retry, plus a serve-seam self-heal that demotes and re-enqueues a dangling `cached` row whose file was lost out-of-band), never the old silent permanent `503`. The TFTP contract in this module is unchanged; the squashfs simply moved from the tree to central's HTTP surface. The operational model — per-device selection, canary pin, server-side rollback, GC, and the `GET /v1/operator/netboot` observability view — is in the [base-image auto-mirror runbook](runbook.md#base-image-auto-mirror-0012).
