...
Status: central, PostgreSQL and the media worker launch together. The full real-media wall demo has passed with two network Players and three recording-renderer Outputs; native rendering has separate Linux evidence. The signed appliance image is built and verified; boot and physical qualification remain in progress. Follow the [delivery checklist](implementation-checklist.md) and [evidence](evidence/README.md).
...
Automatic key-proof enrollment exists at `/v1/enrollment/challenge` and `/v1/enrollment/register`. The single-process Player entry point is `python -m player.service --config /etc/photo-wall/public.json`; see [service configuration and runtime requirements](module-player-service.md). The common appliance image and PXE tree are built; [hosted generic-VM boot](evidence/2026-09-05-github-image.md#first-hosted-exact-image-boot-pass), durable enrollment and restart/rejoin have passed. Physical Pi/PXE qualification remains pending. Startup-only DRM discovery currently requires a Player restart after connector topology changes.
