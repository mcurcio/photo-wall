...
Status: central, PostgreSQL, and the Procrastinate media worker launch together. Earlier two-Player/three-Output and native evidence established the MVP shape, but the complete demo, authenticated browser walkthrough, current exact-image native paths, automatic central rollback, and physical qualification must be rerun after the stateless-Player refactor. Follow the [delivery checklist](implementation-checklist.md) and [evidence](evidence/README.md); do not treat earlier local-state evidence as acceptance of the current design.
