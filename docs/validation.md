...
|---|---|---|---|
| V01 — Fresh PXE startup automatically registers a Player centrally before Frame assignment; replacement preserves Frames and rejects retired bindings. | Central provisioning/registry/sessions; Player equipment; appliance tooling | No persisted configuration or manual per-device setup; common image, central appearance, subsequent binding, stale reconnect | S3, S6 |
...
| V10 — Restart and post-lookahead outages preserve secured assignments, avoid duplicate activation, and reconcile current authorized output. | Durable Runtime/Planner/sessions; Player cache/executor | Fault injection, real-device rejoin, cache exhaustion, diagnosed failures, tested outage duration | S6 |
| V11 — An operator installs and migrates central services, observes automatic Player registration, binds equipment, configures source/Scene/Program, and recovers a failed update. | Central packaging/operator interface; registry; appliance tooling | Clean-environment walkthrough without per-device setup; staged rollout/rollback record | S3–S6; update mechanism scoped before delivery |
...
Restart central and Player components around acquisition, readiness, commitment, and presentation. Continue outages beyond the preparation horizon, exhaust configured cache reserves, then reconnect. Verify current logical playback, no duplicated activation or stale cues, and diagnostic fallback. Repeat after rebinding a Frame and reconnecting the retired Player. Test credentials and binding authority separately from mutable network/display observations.
...
2. Connect and power a fresh, supported network-boot-capable device with no prior Photo Wall enrollment or saved Photo Wall configuration, endpoint, access credentials, cache, or Frame assignment. Verify PXE delivery/launch and automatic central appearance with identity, Outputs, and health before Frame assignment. Use no SSH, per-device configuration, endpoint entry, or manual credential provisioning; automatic credential establishment is permitted.
...
4. Centrally identify Outputs, bind existing Frames, calibrate, and configure source/Scene/Program playback. Reimage/replace a Player, verify automatic appearance, and reject retired bindings on reconnect. Exercise update failure/rollback without reconstructing Frames.
...
Capture boot ID, software/configuration revisions, Outputs/modes, Runs/visible targets/media, cache/readiness, clock uncertainty, intended/observed presentation, resource use, and faults. Detect a live process that stops presenting new content. Results qualify only the tested envelope; record exclusions and rerun affected checks when it changes.
