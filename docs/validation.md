...
### Reaching Central from the netboot (decision 0014)

Stage 1's path to Central ([decision 0014](decisions/0014-reaching-central-from-every-boot-stage.md)) is proven over real sockets and real TLS, with no Pi. The certificates are minted per run with `cryptography`, to the strict X.509 profile the device enforces ([`tests/tls_fixture.py`](../tests/tls_fixture.py)). No key is kept in the tree.

- **DB-free, on every run** ([`tests/test_uplink_tls.py`](../tests/test_uplink_tls.py)): locate through a 301 from http to real Central over TLS, and a TLS-to-TLS hop where SNI and the name check follow the hop. It also covers the https→http downgrade refused with nothing sent to its target, the name mismatches (verify codes 62 and 64), an untrusted CA, and dates before notBefore and after notAfter. Non-HTTP peers, resets, stalls and address fallback are covered in [`tests/test_uplink_transport.py`](../tests/test_uplink_transport.py). A TLS record error mid-body is covered in [`tests/test_uplink_fetch.py`](../tests/test_uplink_fetch.py). The SNTP client runs against the repo's NTP fixture ([`tests/test_uplink_sntp.py`](../tests/test_uplink_sntp.py)).
- **Postgres, CI only** (skipped locally, and reported as skipped): locate, then the direct base fetch, from real Central, digest-checked ([`tests/test_uplink_wire.py`](../tests/test_uplink_wire.py)). The whole real `netboot()` runs from an http root through the 301 to Central over TLS. Every Central stand-in answers `/v1/locate` from the one shared stub ([`tests/test_netboot_e2e_wire.py`](../tests/test_netboot_e2e_wire.py)).
- **Device runtime** (the `netboot-e2e.yml` tracer job): CI runs Python 3.12 with a venv, but the Pi runs Debian trixie's python3 (3.13) and OpenSSL (3.5). So the job stages stage 1's computed closure and mints the certificates on the runner. It then runs [`scripts/uplink_device_harness.py`](../scripts/uplink_device_harness.py) in `debian:trixie-slim` under `python3 -I -S`, with only the closure on `sys.path`. The harness imports every closure module, then runs the 301 chain, a TLS-to-TLS hop, the downgrade and the 62/64 rows against stdlib servers. The same harness runs on every PR on CI's own Python ([`tests/test_uplink_device_harness.py`](../tests/test_uplink_device_harness.py)).
- **Closure** (every PR): stage 1's computed closure reaches `uplink` and no provisioner or Player code. Every closure module imports with only the closure and the stdlib ([`tests/test_netboot_closure.py`](../tests/test_netboot_closure.py)).

The base-image build runs the initrd content-verify against the closure manifest it wrote, and the kernel-config check. Hardware qualification is pending: the M0 and M1 observations of [decision 0014's delivery](decisions/0014-reaching-central-from-every-boot-stage.md#delivery) have no recorded result yet.
