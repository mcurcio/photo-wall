...
def composition(project: str) -> dict:
...
class BootFixture:
    def __init__(self, state: Path, *, run=command):
        ...
        self.run = run
        require(read_json(self.state/"compose.json") == composition(self.project), "compose_changed")
    ...
    def prepare(cls, state: Path, bundle: Path, deployment: Path, central_image: str):
        ...
        try:
            ...
            copied = {}
            for name in SOURCES:
                target_name = name.removeprefix("scripts/")
            ...
            write_json(state/"compose.json",composition(project))
            return cls(state)
    ...
    def check_inputs(self):
        ...
        for relative,expected in self.marker["inputs"].items():
            require(relative.split("/")[0] in ("bundle","public","tls") and ".." not in Path(relative).parts,
                    "input_path_invalid")
        ...
        with self.locked():
            ...
            if not self.marker["initialized"]:
                ...
                if self.exists("container",seed["name"]):
                    self.remove(seed)
                    seed = self.remember("container",self.project+"-seed")
                mounts = [argument for name in ("bundle","public","tls","probe") for argument in
                ...
                self.run(["docker","create","--name",seed["name"],"--label",LABEL+"="+self.project,
                          ...
                          *mounts,image["name"],"python","/opt/boot-fixture/runtime.py","init",
                          encoded(self.expect()).decode().strip()],timeout=30)
                self.check(seed)
                try:
                    for name in ("bundle","public","tls"):
                        self.check(seed)
            ...
            containers = [self.remember("container",self.project+"-"+name)
                          for name in ("database","central","ntp")]
            try:
