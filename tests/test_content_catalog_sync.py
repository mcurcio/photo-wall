...
import pytest
from catalog_fakes import InMemoryDeviceRecords, InMemoryReleaseRecords, device, release_row
...
def world(origin_listing, *, releases=(), devices=(), promoted=None, etag=None, bound=0,
          assets: InMemoryAssetRecords | None = None, **handler_options) -> World:
...
@pytest.mark.parametrize("promoted,bound,expected", [
    (None, 3, T),    # nothing promoted: promote regardless of bound players
    (T1, 0, T),      # no bound player: follow the newest
    (T1, 3, T1),     # a configured fleet keeps the operator's choice
])
def test_auto_promote(promoted, bound, expected):
