...
def wheel_record(name, tag="py3-none-any"):
    return {
        ...
        "hash": "sha256:" + hashlib.sha256(PAYLOAD).hexdigest(), "size": len(PAYLOAD),
    }


@pytest.fixture
def inputs():
    project = {"project": {"version": "0.1.0", "requires-python": ">=3.12,<3.13",
                           "dependencies": [f"{name}==1.0" for name in sorted(package.ROOTS)]}}
    lock = {"version": 1, "revision": 3, "requires-python": "==3.12.*", "package": [
        {"name": name, "version": "1.0", "source": {"registry": "https://pypi.org/simple"},
         "wheels": [wheel_record(name)]} for name in sorted(package.ROOTS)
...
def source_files():
    return {"player/__init__.py": b"", "player/service.py": b"import httpx\n",
            "contracts/__init__.py": b"", "contracts/models.py": b"import pydantic\n",
            "pyproject.toml": b"", "uv.lock": b""}
...
def test_player_wheel_is_deterministic_and_has_only_runtime_packages(inputs):
    ...
    assert set(path.split("/")[0] for path in contents) == {
        "player", "contracts", f"photo_wall_player-{VERSION}.dist-info"}
...
@pytest.mark.parametrize("path", ["central/app.py", "media/worker.py", "player/../private.py",
                                  "player/config.json", "photo-wall.data/secret"])
...
@pytest.mark.parametrize("alteration", ["extra_dep", "extra_header", "duplicate_name", "tag",
                                         "record", "source"])
...
def test_target_tags_prefer_arm64_cp312_and_support_abi3(inputs):
    ...
    locked["wheels"] = [wheel_record(locked["name"], tag) for tag in (
        "cp312-cp312-manylinux_2_17_x86_64", "cp313-cp313-manylinux_2_17_aarch64",
        "cp312-cp312-musllinux_1_2_aarch64", "cp312-cp312-manylinux_2_40_aarch64",
        "cp312-cp312-manylinux_2_17_aarch64", "py3-none-any")]
...
def test_dependency_markers_are_target_specific_and_traversal_is_closed(inputs):
    ...
    lock["package"].append({"name": "linux-only", "version": "1.0",
...
@pytest.mark.parametrize("fault", [
    "format", "python", "tool", "duplicate", "source", "missing_hash", "wrong_hash",
    "size", "wrong_name", "wrong_version", "host", "redirect_query", "no_compatible",
    "missing_dependency", "forbidden", "extra", "unknown_marker", "version_conflict",
    "root_range", "root_extra", "root_marker", "missing_root",
])
def test_invalid_locks_fail_closed(inputs, fault):
    ...
    if fault == "format":
        ...
        wheel["url"] = wheel["url"].replace("-1.0-", "-2.0-") if fault == "wrong_version" else (
            "https://files.pythonhosted.org/x/not_the_package-1.0-py3-none-any.whl")
    elif fault in ("host", "redirect_query"):
        wheel["url"] = wheel["url"].replace("files.pythonhosted.org", "example.com") if (
            fault == "host") else wheel["url"] + "?token=foo"
    elif fault == "no_compatible":
        first["wheels"] = [wheel_record(first["name"], "cp312-cp312-win_amd64")]
    elif fault in ("missing_dependency", "forbidden", "extra", "unknown_marker",
                   "version_conflict"):
        ...
        project["project"]["dependencies"][0] = {
            "root_range": "cryptography>=1.0", "root_extra": "cryptography[foo]==1.0",
...
@pytest.mark.parametrize("path", ["../private.py", "/etc/passwd", "player//foo.py",
                                  "central/app.py", "player/key.pem", "player/.private.py"])
...
@pytest.mark.parametrize("chunks", [[PAYLOAD + b"!"], [PAYLOAD[:-1]], [b"x" * len(PAYLOAD)],
                                    ["not bytes"]])
...
def test_build_uses_commit_ignores_dirty_private_source_and_inventories_hashes(committed_repo, tmp_path):
...
def test_git_subprocess_has_deadline_and_reports_failed_commands():
    with pytest.raises(package.BuildError, match="time limit"):
        package.bounded_process([sys.executable, "-c", "import time;time.sleep(60)"],
                                limit=32, timeout=0.05)
